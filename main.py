...
def predict_image(img_path, threshold=0.5):
    ...
    print(f"Prediction (Raw): {prediction}")  # Debugging line
...
test_generator = test_datagen.flow_from_directory(
    ...
    batch_size=32,
    class_mode='binary',
    shuffle=False  # To ensure predictions are in the correct order
)
...
print(f"Sample raw predictions: {predictions[:5]}")  # Print the first 5 predictions

# Visualize some images
sample_batch = next(train_generator)
for i in range(5):
    plt.imshow(sample_batch[0][i])  # Display the first 5 images
    plt.title(f"Label: {'Dog' if sample_batch[1][i] == 1 else 'Cat'}")
    plt.show()
